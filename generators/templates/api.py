...
def generate_constants(project_name: str, lib_path: Path):
    generate_file(project_name, lib_path, "apis/common/constants_template.dart", "apis/common/constants.dart")

def generate_auth_interceptor(project_name: str, lib_path: Path):
    generate_file(project_name, lib_path, "apis/interceptors/auth_interceptor_template.dart", "apis/interceptors/auth_interceptor.dart")

def generate_storage_repository(project_name: str, lib_path: Path):
    generate_file(project_name, lib_path, "infrastructure/storage/storage_repository_template.dart", "infrastructure/storage/storage_repository.dart")

def generate_logger(project_name: str, lib_path: Path):
    generate_file(project_name, lib_path, "logging/logger_template.dart", "logging/logger.dart")

def generate_console(project_name: str, lib_path: Path):
    generate_file(project_name, lib_path, "logging/console_template.dart", "logging/console.dart")

def generate_analytics_logging(project_name: str, lib_path: Path):
    generate_file(project_name, lib_path, "logging/analytics_logging_template.dart", "logging/analytics_logging.dart")

def generate_logger_injectable_module(project_name: str, lib_path: Path):
    generate_file(project_name, lib_path, "logging/logger_injectable_module_template.dart", "logging/logger_injectable_module.dart")

def generate_api_logger(project_name: str, lib_path: Path):
    generate_file(project_name, lib_path, "apis/interceptors/api_logger_template.dart", "apis/interceptors/api_logger.dart")

def generate_api_injectable_module(project_name: str, lib_path: Path):
    generate_file(project_name, lib_path, "apis/core/api_injectable_module_template.dart", "apis/core/api_injectable_module.dart")